...
## ⚠️ OPERACIONAL — Stripe (checkout)
- **Um produto ATIVO por plano.** Um `Price` anexado a um `Product` arquivado **não pode ser comprado** ("product is not active") → `checkout.sessions.create` dá 400 → a rota `/api/signup/checkout` responde 500 e a página do Stripe não abre. Foi o bug de 2026-06-30 (os preços `_v2`/`_v3` haviam sido anexados a produtos arquivados na materialização). **Correção de dados:** reativados os 3 produtos (Oficina/Loja/Potência) no Stripe LIVE. **Blindagem:** `ensureProduct` ([lib/stripe.ts](src/lib/stripe.ts)) busca com `active:'true'`. Ao mexer em preços no futuro, **arquive só o Price antigo, nunca o Product** que ainda tem preços em uso.
...
## ⏳ Outras pendências
- ~~**Atribuição de papel a usuário**~~ (resolvido: Usuários & acessos define o papel; Administrador é fixo) — texto histórico: não havia UI para definir `User.role` (ex.: tornar alguém `hospitaller`). Hoje admin/venerável já acessam a Hospitalaria; para um Hospitaleiro dedicado, falta uma tela de gestão de usuários/papéis (ou setar `role` direto no banco).
...
## Sessão 2026-09-19/20 — importador financeiro e regras de acesso (NÃO refazer)
- ✅ **Importador de backup financeiro** (`src/lib/legacy-import/*`, `api/import/financial/*`, tela `configuracoes/importar-financeiro`, `scripts/import-legacy-backup.ts`, migration `20260919210000_add_balancete_import_fields`). Razão × extrato por (data, valor); abertura de saldo → `openingBalance`; XLSX de PDF realinhados; conferência de totais antes de gravar; lote desfazível. Importado na loja de teste Tim Maia (`horus-reaa`, membros e contrapartes com sufixo " (TESTE)": `scripts/tag-test-lodge-members.ts`). Manual v1.32 (6.10).
- ✅ **Administrador fixo** (`lib/admin-policy.ts`): sem promoção/rebaixamento, `POST /api/users` cria Administrador (teto 2 ativos), um e-mail = um papel (membro × Administrador), `grant-access` não religa mais o Administrador a membro, matriz não restringe o Administrador, **Minha conta** (`api/account/profile`, `confirm-email`, `lib/email-change-token.ts`). `scripts/separate-admin-from-member.ts` aplicado na `amm139`. Manual v1.33 (caps. 2, 3, 6.3, 6.11, FAQ).
- ✅ **Trava de lojas reais** nos scripts de escrita: `scripts/protected-lodges.ts` (`amm139`) — exige `--i-know-this-is-a-real-lodge`.
- ⚠️ Scripts contra produção: no PowerShell, `Remove-Item Env:DATABASE_URL, Env:APP_DATABASE_URL` antes (a env do usuário aponta para localhost e o `--env-file` não sobrescreve).
