...
- [x] **Float → Decimal: decidido NÃO migrar** (2026-09-20). Auditoria em produção (`scripts/audit-money-decimals.ts`, somente leitura): 28 campos, ~2.000 linhas de dinheiro, **nenhum valor com mais de 2 casas, NaN/Infinity ou grande demais**. Migrar geraria risco silencioso no cliente (valor vira texto no JSON). Em vez disso: 12 pontos da API que aceitavam dinheiro sem checar casas (ativos, orçamento, campanhas, doações, aporte a campanha, saldo inicial, transferências, cobranças individual/em lote) agora validam com `hasAtMostCents`/`isValidMoney` (`lib/money.ts`) e respondem 400. Rodar a auditoria de novo de tempos em tempos. Opcional: `CHECK` no banco como reforço (cuidado com os importadores — legacy-import/OFX — que não passam por essa validação).
