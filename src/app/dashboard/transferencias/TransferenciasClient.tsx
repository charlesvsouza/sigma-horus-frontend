                ...
                <div key={t.id} className="rounded-lg border border-white/5 bg-sigma-blue-deep/50 px-4 py-3">
                  ...
                  {t.status === 'pending' && canApprove ? (
                    <div className="mt-2 flex items-center gap-3">
                      ...
                      <button disabled={decidingId === t.id} onClick={() => void decide(t.id, 'reject')} className="text-xs px-1 py-1 text-rose-300 transition hover:text-rose-200 disabled:opacity-40">Rejeitar</button>
                    </div>
                  ) : null}
                </div>
              ))}
