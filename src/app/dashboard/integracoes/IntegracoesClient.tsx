...
export default function IntegracoesClient({ asaas, messaging }: { asaas: AsaasStatus; messaging: MsgStatus }) {
  ...
  const webhookFullUrl = typeof window !== 'undefined' ? `${window.location.origin}${asaas.webhookUrl}` : asaas.webhookUrl;
          ...
          <form onSubmit={save} className="mt-5 space-y-4">
            <div>
              ...
              <input
                ...
                placeholder={asaas.configured ? 'Cole uma nova chave para substituir' : '$aact_...'}
              />
            </div>
            <div>
            ...
            <div className="flex flex-wrap gap-3">
              <Button type="submit" disabled={saving}>
                {saving ? 'Validando…' : asaas.configured ? 'Atualizar chave' : 'Conectar Asaas'}
              </Button>
              {asaas.configured && (
